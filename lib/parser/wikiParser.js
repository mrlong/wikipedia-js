...
function parseJsonTxt(wiki, callback) {
  if (wiki.query && wiki.query.pages) {
    var keys = Object.keys(wiki.query.pages);
    if (keys.length > 0) {
...
//      console.log(latestArticle);
      ...
      linesToParse.forEach(function (sectionLines) {
        ...
        var b = new RegExp("\\]\\]","g");
        var c = new RegExp("'","g");
        var d = new RegExp("/<ref[^\/]*\/>|<ref[^>]*>[^<]*<\/ref>","g");
        var e = new RegExp("/<!--\s*[^-]*-->","g");
        var f = new RegExp('/<ref\s*name="([^"]*)"\s*\/?>[^<]*(<\/ref>)?',"g");
        sectionLines.forEach(function (line) {
          parsedText += line.replace(a,'').replace(b,'').replace(c,'')
